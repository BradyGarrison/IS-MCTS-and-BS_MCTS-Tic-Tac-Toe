...
class TreeNode(NodeMixin):
    def __init__(self, BSMCTSNode, parent=None, name = None):
        ...
        self.visits = BSMCTSNode.visits
        self.reward = BSMCTSNode.reward
        self.color = BSMCTSNode.color
        self.selected_action = maxNodeRewardEstimation(BSMCTSNode, BSMCTSNode.beliefs[0])
        self.parent_action = BSMCTSNode.parent_action
        self.parent = parent
        self.name = name
...
def createTree():
    ...
    root_node = BSMCTSNode(root_node = True, color = 'X', beliefState = [Belief(x, 1)])
    ...
    DotExporter(test,options = ["ratio = 1.5"], nodenamefunc = nodenamefunc, nodeattrfunc=nodeattrfunc, edgeattrfunc = edgeattrfunc).to_dotfile("BSMCTS v7.dot")
